import { ERC20TokenAmount } from "generated";
import {
...
// Used by: UNI, COMP, GTC tokens (use "amount" instead of "value")
// ============================================================
ERC20TokenAmount.Transfer.handler(async ({ event, context }) => {
...
// DelegateChanged
// ============================================================
ERC20TokenAmount.DelegateChanged.handler(async ({ event, context }) => {
  const daoId = resolveDaoFromTokenAddress(event.srcAddress);
  if (!daoId) return;
...
// ============================================================
ERC20TokenAmount.DelegateVotesChanged.handler(async ({ event, context }) => {
  const daoId = resolveDaoFromTokenAddress(event.srcAddress);
  if (!daoId) return;
